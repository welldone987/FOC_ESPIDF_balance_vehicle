 ...
 * Start()/Suspend()随电机初始化与锁存停机管理DMA；回调保存带交付时间的原始帧。
 * Read()清理DMA池后原子取最新帧，用官方解析接口提取最后扫描并按通道归位。
...
struct Sample {
    ...
    // started_us为该帧交付时间减标称扫描跨度，单位us；不含未实测的中断延迟。
...
// Read()排空DMA池并消费最新回调帧；无新帧/帧无效时返回超时，避免重复使用旧样本。
