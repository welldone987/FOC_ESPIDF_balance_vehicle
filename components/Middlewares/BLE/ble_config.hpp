...
// DeviceName是BLE广播名称，网页控制端按此识别设备。
inline constexpr char DeviceName[] = "平衡车";
// ServiceUuid、CommandUuid和TelemetryUuid与网页控制端的整数协议保持一致。
...
// ReadyTimeout_us是NimBLE主机就绪的内部等待上限。
// StartupWait_ms是app_main等待BleTask报告的上限。
