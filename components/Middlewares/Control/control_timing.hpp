...
// waiting与outputs_off为保留枚举值（无赋值点），编号不可重排。
...
struct ControlTiming {
    // cycle、notifications和skipped_releases记录周期、通知与本轮合并数。
    std::uint32_t cycle{}, notifications{}, skipped_releases{};
    // stage保存故障定格时已进入的阶段。
    ControlStage stage{};
    // driving标记目标时效有效；imu_updated记录本轮是否读取IMU；first_release标记首次控制周期。
    bool driving{}, imu_updated{}, first_release{};
    // dt_us、elapsed_us和各阶段耗时分别记录周期间隔与执行耗时，单位us。
    std::int64_t dt_us{}, elapsed_us{}, encoder_us{}, imu_us{}, outer_us{};
    // current保存电流环内部的阶段计时。
