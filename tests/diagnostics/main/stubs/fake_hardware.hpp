...
inline std::uint16_t encoder_count=0;
...
inline void reset() { operation=fail_operation=raw_reads=0; fail_channel=convert_channel=-1; noise=0; now=1000; read_delay=pwm_delay=encoder_read_delay=math_delay=0; observed_current_timing=nullptr; pwm_calls=encoder_count=0; gpio_error=enable_calls=0; driver_failure=align_fail=-1; align_calls=i2c_error=0; for(auto &v:mv)v=1650; for(auto &v:imu_raw)v=0; for(auto &v:imu_registers)v=0; }
}
...
struct i2c_config_t {int mode,sda_io_num,scl_io_num,sda_pullup_en,scl_pullup_en;struct {unsigned clk_speed;}master;unsigned clk_flags;};
...
inline int i2c_bus_read_bytes(void*,int,int length,std::uint8_t *raw){
    if (length==12) { for(int i=0;i<length;++i)raw[i]=fake::imu_raw[i]; }
    else { fake::now+=fake::encoder_read_delay; raw[0]=fake::encoder_count>>8; raw[1]=fake::encoder_count&255; }
    return fake::i2c_error;
}
inline int i2c_bus_read_byte(void*,int reg,std::uint8_t *raw){*raw=fake::imu_registers[reg];return fake::i2c_error;}
inline int i2c_bus_write_byte(void*,int reg,std::uint8_t value){if(fake::i2c_error==ESP_OK)fake::imu_registers[reg]=value;return fake::i2c_error;}
...
class BLDCDriver3PWM {public: BLDCDriver3PWM(int,int,int){} float voltage_power_supply{},voltage_limit{}; int init(int i){return i!=fake::driver_failure;} void disable(){} void setPwm(float,float,float){++fake::pwm_calls;fake::now+=fake::pwm_delay;} };
