...
基于 **DengFOC V4** 与 ESP32-WROOM-32 的两轮自平衡车原生 C++ 固件，使用 **ESP-IDF v6.0.2** 构建。`docs/` 下的本地工程文档与厂商资料不进入 Git，仅保留浏览器控制界面。
...
- **FOC 电机驱动** — 两台 7 极对无刷电机按板级通道 M0/M1 编号，各使用一只 AS5600 编码器；`espressif/esp_simplefoc` 用于启动对齐，运行电流环由 BSP 负责
...
当前源码设置 `CurrentHardwareVerified=true`，因此启动时允许拉高 GPIO12，并依次执行 M1、M0 对齐。它只是软件放行门，不是实板验证证据；对齐可能转动车轮，且此阶段尚未进入周期电流保护。
...
当前源码已于 2026-09-13 使用 ESP-IDF v6.0.2 执行 `idf.py --no-ccache build` 并通过，应用镜像大小为 `0x13d570` 字节，最小应用分区剩余 59%。这只证明编译与镜像容量，不证明时序、接线、电机安全或整车稳定性。

正常构建流程不包含烧录或电机运行。只有在明确授权硬件操作并选定实际串口后，才执行 `idf.py -p COMx flash monitor`。
...
每次推送到 `main` 或提交 PR 时，CI 会使用 ESP-IDF v6.0.2 完成整包构建。已跟踪的[浏览器控制界面](docs/平衡车控制界面.html)提供 BLE 操作入口。
